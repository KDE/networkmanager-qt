...
public:
    ...
    Q_INVOKABLE NetworkManager::Connection *findConnection(const QString &path) const;
    /* Retrieves the connection for the given @p uuid, returns null if not found*/
    Q_INVOKABLE NetworkManager::Connection *findConnectionByUuid(const QString &uuid);
    /*! Retrieves the connection for the given path, returns null if not found */
    Q_INVOKABLE NetworkManager::Connection::List listConnections() const;
