...
public:
     ...
     * are available an error is returned. \value AllowInteraction Allows the request to interact with the user, possibly prompting via UI for secrets if any
     * are required, or if none are found in persistent storage. \value RequestNew Explicitly prompt for new secrets from the user. This flag signals that
     * NetworkManager thinks any existing secrets are invalid or wrong. This flag implies that interaction is allowed. \value UserRequested Set if the request
     ...
     * and the \a callMessage is ONLY needed if \ref setDelayedReply()
